...
class CDontPhaseMeBro : public CPlugIn
{
public:
	...
	float calculateAPFCutoffFreq(float fLFOSample);

	//functions to calc BiQuad coeffs (might not need these)
	void calculateModAPFCoeffs(float fLFOSample);

	//for non modded cutoff calc
	void calculateFirstOrderAPFCoeffs(float fCutoffFreq, CBiQuad * pBiQuadFilter);
	void calculateSecondOrderAPFCoeffs(float fCutoffFreq, CBiQuad * pBiQuadFilter);
