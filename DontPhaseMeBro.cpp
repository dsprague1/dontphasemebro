...
*/
CDontPhaseMeBro::CDontPhaseMeBro()
{
...
*/
CDontPhaseMeBro::~CDontPhaseMeBro(void)
{
}
...
*/
bool __stdcall CDontPhaseMeBro::initialize()
{
	// Add your code here
...
*/
bool __stdcall CDontPhaseMeBro::prepareForPlay()
{
	// Add your code here:
	m_APF.flushDelays();
	calculateFirstOrderAPFCoeffs(m_fCornerFreq, &m_APF);

	//setup LFO
	m_LFO.m_fFrequency_Hz = m_fLFORate;
	m_LFO.m_uPolarity = 1;
	m_LFO.m_uTableMode = 0;
	m_LFO.m_uOscType = m_uLFOWaveform;
	m_LFO.setSampleRate(m_nSampleRate);
	return true;
}

float CDontPhaseMeBro::calculateAPFCutoffFreq(float fLFOSample)
{return (m_fModDepth/100.0)*(fLFOSample*(m_fMaxCutoff - m_fMinCutoff)) + m_fMinCutoff;}

void CDontPhaseMeBro::calculateFirstOrderAPFCoeffs(float fCutoffFreq, CBiQuad * pBiQuadFilter)
{
	//coeff calculation
	float alpha_num = tan(pi*fCutoffFreq/(float)m_nSampleRate) - 1.0;
	float alpha_den = tan(pi*fCutoffFreq/(float)m_nSampleRate) + 1.0;
...
void CDontPhaseMeBro::calculateSecondOrderAPFCoeffs(float fCutoffFreq, CBiQuad * pBiQuadFilter)
{
	//coeff calculation
	float alpha_num = tan(pi*m_fQ/(float)m_nSampleRate) - 1.0;
	float alpha_den = tan(pi*m_fQ/(float)m_nSampleRate) + 1.0;
	float alpha = alpha_num/alpha_den;
	float theta_c = (2*pi*fCutoffFreq)/((float)m_nSampleRate);
	float beta = -cos(theta_c);

	pBiQuadFilter->m_f_a0 = -alpha;
	pBiQuadFilter->m_f_a1 = beta*(1 - alpha);
	pBiQuadFilter->m_f_a2 = 1.0;
	pBiQuadFilter->m_f_b1 = beta*(1 - alpha);
	pBiQuadFilter->m_f_b2 = -alpha;
}

void CDontPhaseMeBro::calculateModAPFCoeffs(float fLFOSample)
{
	float fCutoffFreq = calculateAPFCutoffFreq(fLFOSample);

	if(m_uFiltOrder == FIRST)
		calculateFirstOrderAPFCoeffs(fCutoffFreq, &m_APF);
	else
		calculateSecondOrderAPFCoeffs(fCutoffFreq, &m_APF);
}
...
*/
bool __stdcall CDontPhaseMeBro::processAudioFrame(float* pInputBuffer, float* pOutputBuffer, UINT uNumInputChannels, UINT uNumOutputChannels)
{
	...
	//save on processing if no mod
	if(m_uModSource != NONE)
	{
		float fYn = 0;
		float fYqn = 0;
		float fDepth = m_fModDepth/100.0;
		...
		calculateModAPFCoeffs(fYn);
	}

	float fAPF_OutL = m_APF.doBiQuad(xn_L);
	float fAPF_OutR = m_APF.doBiQuad(xn_R);

	pOutputBuffer[0] = (m_fDryWet/100.0)*fAPF_OutL + (1.0 - (m_fDryWet/100.0))*pInputBuffer[0];

	// Mono-In, Stereo-Out (AUX Effect)
	if(uNumInputChannels == 1 && uNumOutputChannels == 2)
		pOutputBuffer[1] = (m_fDryWet/100.0)*fAPF_OutL + (1.0 - (m_fDryWet/100.0))*pInputBuffer[0];

	// Stereo-In, Stereo-Out (INSERT Effect)
	if(uNumInputChannels == 2 && uNumOutputChannels == 2)
		pOutputBuffer[1] = (m_fDryWet/100.0)*fAPF_OutR + (1.0 - (m_fDryWet/100.0))*pInputBuffer[1];
...
//
bool __stdcall CDontPhaseMeBro::userInterfaceChange(int nControlIndex)
{
	...
	switch(nControlIndex)
	{
		...
		case 2:
		{
			if(m_uFiltOrder == FIRST)
				...
				calculateSecondOrderAPFCoeffs(m_fCornerFreq, &m_APF);
			break;
		}
		default:
			break;
...
*/
bool __stdcall CDontPhaseMeBro::joystickControlChange(float fControlA, float fControlB, float fControlC, float fControlD, float fACMix, float fBDMix)
{
	// add your code here
...
*/
bool __stdcall CDontPhaseMeBro::processRackAFXAudioBuffer(float* pInputBuffer, float* pOutputBuffer,
...
*/
bool __stdcall CDontPhaseMeBro::processVSTAudioBuffer(float** inBuffer, float** outBuffer, UINT uNumChannels, int inFramesToProcess)
{
...
bool __stdcall CDontPhaseMeBro::midiNoteOn(UINT uChannel, UINT uMIDINote, UINT uVelocity)
{
	return true;
}

bool __stdcall CDontPhaseMeBro::midiNoteOff(UINT uChannel, UINT uMIDINote, UINT uVelocity, bool bAllNotesOff)
{
	return true;
}

// uModValue = 0->127
bool __stdcall CDontPhaseMeBro::midiModWheel(UINT uChannel, UINT uModValue)
{
	return true;
}
...
bool __stdcall CDontPhaseMeBro::midiPitchBend(UINT uChannel, int nActualPitchBendValue, float fNormalizedPitchBendValue)
{
	return true;
}
...
*/
bool __stdcall CDontPhaseMeBro::midiClock()
{
...
bool __stdcall CDontPhaseMeBro::midiMessage(unsigned char cChannel, unsigned char cStatus, unsigned char
...
bool __stdcall CDontPhaseMeBro::initUI()
{
	// ADDED BY RACKAFX -- DO NOT EDIT THIS CODE!!! ------------------------------ //
...
// **--0xDEA7--**
	...
	ui0->fInitUserFloatValue = 0.010000;
	...
	ui1->m_pUserCookedUINTData = &m_uLFOWaveform;
	ui1->cControlUnits = "                                                                ";
	ui1->cVariableName = "m_uLFOWaveform";
	ui1->cEnumeratedList = "SINE,TRI,SAW,SQUARE";
	...
	ui2->fUserDisplayDataHiLimit = 20000.000000;
	...
	ui3->fInitUserFloatValue = 0.100000;
	ui3->fInitUserDoubleValue = 0;
	ui3->fInitUserUINTValue = 0;
	ui3->m_pUserCookedIntData = NULL;
	ui3->m_pUserCookedFloatData = &m_fQ;
	ui3->m_pUserCookedDoubleData = NULL;
	ui3->m_pUserCookedUINTData = NULL;
	ui3->cControlUnits = "                                                                ";
	ui3->cVariableName = "m_fQ";
	ui3->cEnumeratedList = "SEL1,SEL2,SEL3";
	ui3->dPresetData[0] = 0.100000;ui3->dPresetData[1] = 0.000000;ui3->dPresetData[2] = 0.000000;ui3->dPresetData[3] = 0.000000;ui3->dPresetData[4] = 0.000000;ui3->dPresetData[5] = 0.000000;ui3->dPresetData[6] = 0.000000;ui3->dPresetData[7] = 0.000000;ui3->dPresetData[8] = 0.000000;ui3->dPresetData[9] = 0.000000;ui3->dPresetData[10] = 0.000000;ui3->dPresetData[11] = 0.000000;ui3->dPresetData[12] = 0.000000;ui3->dPresetData[13] = 0.000000;ui3->dPresetData[14] = 0.000000;ui3->dPresetData[15] = 0.000000;
	ui3->cControlName = "Resonance";
	...
	ui4->fUserDisplayDataHiLimit = 100.000000;
	...
	ui4->m_pUserCookedFloatData = &m_fModDepth;
	ui4->m_pUserCookedDoubleData = NULL;
	ui4->m_pUserCookedUINTData = NULL;
	ui4->cControlUnits = "                                                                ";
	ui4->cVariableName = "m_fModDepth";
	...
	ui5->m_pUserCookedFloatData = &m_fDryWet;
	ui5->m_pUserCookedDoubleData = NULL;
	ui5->m_pUserCookedUINTData = NULL;
	ui5->cControlUnits = "                                                                ";
	ui5->cVariableName = "m_fDryWet";
	...
	ui6->m_pUserCookedUINTData = &m_uFiltOrder;
	ui6->cControlUnits = "Units                                                           ";
	ui6->cVariableName = "m_uFiltOrder";
	ui6->cEnumeratedList = "FIRST,SECOND";
	...
	ui7->fUserDisplayDataHiLimit = 2.000000;
	...
	ui7->m_pUserCookedUINTData = &m_uModSource;
	ui7->cControlUnits = "";
	ui7->cVariableName = "m_uModSource";
	ui7->cEnumeratedList = "NONE,LFO,SELF";
